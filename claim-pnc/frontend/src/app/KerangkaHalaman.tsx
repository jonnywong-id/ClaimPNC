...
type Entri = { ke: string; label: string; ikon: ReactNode }
...
const menu: Entri[] = [
  ...
  { ke: '/master/ambang-komite', label: 'Ambang Komite', ikon: <IkonTangga className="h-4 w-4" /> },
  {
    ke: '/komite/penjenjangan',
    label: 'Penjenjangan Komite',
    ikon: <IkonTimbangan className="h-4 w-4" />,
  },
]

/**
 * KerangkaHalaman membungkus layar yang berada di balik sesi: bilah atas, menu, dan isi.
 *
 ...
 */
export function KerangkaHalaman({ anak }: { anak: ReactNode }) {
  return (
      ...
      <div className="mx-auto max-w-6xl px-4">
        <div className="flex h-16 items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {pengguna && <ChipPengguna nama={pengguna.nama} jenis={pengguna.jenis} identitas={pengguna.identitas} />}
 ...
 */
function ChipPengguna({ nama, jenis, identitas }: { nama: string; jenis: string; identitas: string }) {
  return (
      ...
      <div className="hidden min-w-0 sm:block">
        ...
        */}
        ...
        <p className="max-w-[10rem] truncate text-xs leading-tight text-slate-500">
          {jenis === JenisPengguna.karyawan ? `NIK ${identitas}` : identitas}
        </p>
      </div>
    </div>
  )

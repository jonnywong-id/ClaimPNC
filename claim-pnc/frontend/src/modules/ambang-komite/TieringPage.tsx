...
import { KomiteMode, type KomiteApprover, type KomiteTieringResponse } from '@/api/tipe'
import { IkonPeringatan, IkonTimbangan } from '@/components/Ikon'
        ...
        <div className="grid gap-4 sm:grid-cols-[1fr_auto] sm:items-end">
          <div className="grid gap-4 sm:grid-cols-2">
            <KolomIsian
              ...
              galat={fieldError}
              petunjuk="Boleh diketik dengan titik ribuan."
            />
          ...
          <Tombol nada="utama" type="submit" disabled={result.isFetching}>
            <IkonTimbangan className="h-4 w-4" />
        ...
        <div className="p-5">
          ...
          */}
          {(result.tersingkir?.length ?? 0) > 0 ? (
            <PesanGalat
              judul="Seluruh penyetuju tersingkir"
              keterangan={
            ...
            <PesanGalat
              judul="Tidak ada jenjang yang cocok"
              keterangan={
...
function ExcludedNote({ excluded }: { excluded: KomiteApprover[] }) {
  return (
    <div className="flex items-start gap-3 border-t border-amber-200 bg-amber-50/70 p-4">
      <IkonPeringatan className="mt-0.5 h-4 w-4 shrink-0 text-amber-700" />
  ...
  if (error instanceof GalatJaringan) {
    return (
      <PesanGalat
        ...
        keterangan="Perhitungan belum dapat dijalankan. Periksa koneksi lalu tekan Hitung lagi."
        nada="gangguan"
      />
    )
  }
  if (error instanceof GalatAPI) {
    return <PesanGalat judul="Tidak dapat menghitung" keterangan={error.message} nada="penolakan" />
  }
  return (
    <PesanGalat
      judul="Tidak dapat menghitung"
      keterangan="Perhitungan belum dapat dijalankan. Coba tekan Hitung lagi."
      nada="gangguan"
    />
  )
}

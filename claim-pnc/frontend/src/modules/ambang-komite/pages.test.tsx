...
describe('layar Penjenjangan Komite', () => {
  ...
  it('menolak nilai yang bukan angka tanpa memanggil server', async () => {
    ...
    // Pencocokannya menyertakan "misalnya" supaya ia hanya kena PESAN GALAT-nya. Tanpa
    // itu, polanya juga kena paragraf pengantar layar yang kebetulan dibuka kalimat yang
    // sama ("Masukkan nilai klaim dan lini bisnisnya untuk melihat…"), sehingga ujinya
    // gugur karena menemukan dua elemen — bukan karena galatnya tidak muncul.
    expect(await screen.findByText(/Masukkan nilai klaim, misalnya/)).toBeInTheDocument()
    expect(calls.some((p) => p.url.startsWith(PENJENJANGAN_PATH))).toBe(false)
  })
  ...
  it('mengisi pilihan lini dari master', async () => {
    ...
    // Yang ditunggu adalah OPSINYA, bukan select-nya. `findByLabelText` sudah selesai
    // begitu select-nya tergambar — padahal isinya baru tiba setelah master dijawab
    // server, sehingga menagih opsi saat itu juga adalah adu cepat yang kadang kalah.
    await within(select).findByRole('option', { name: 'NONMBU' })

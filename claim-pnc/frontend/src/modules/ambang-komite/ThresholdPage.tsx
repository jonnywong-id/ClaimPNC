...
import type { KomiteThreshold, KomiteFinding } from '@/api/tipe'
import { IkonMuatUlang, IkonPeringatan } from '@/components/Ikon'
 ...
 *
 * Nama prop komponen bersama (`kolom`, `baris`, `judul`, …) tetap berbahasa Indonesia:
 * komponen itu milik modul lain yang belum diganti namanya, dan `D-81` hanya mengecualikan
 * nama modul — bukan mengizinkan modul ini mengubah milik modul lain.
 */
export function ThresholdPage() {
  ...
  const columns: Kolom<KomiteThreshold>[] = [
    {
      kunci: 'lini',
      judul: 'Lini bisnis',
      nilai: (a) => a.lini,
      lebar: '13%',
      tampil: (a) => <span className="font-medium text-slate-900">{a.lini}</span>,
    },
    {
      kunci: 'jenjang',
      judul: 'Jenjang',
      nilai: (a) => String(a.jenjang),
      lebar: '8%',
    },
    {
      kunci: 'batas_bawah',
      judul: 'Mulai nilai',
      nilai: (a) => a.batas_bawah,
      lebar: '16%',
      tampil: (a) => (
    ...
    },
    {
      kunci: 'jenis_komite',
      judul: 'Jenis',
      nilai: (a) => a.jenis_komite,
      lebar: '8%',
      tampil: (a) => <span className="tabular-nums text-slate-600">{a.jenis_komite || '—'}</span>,
    },
    {
      kunci: 'status',
      judul: 'Status',
      nilai: (a) => rowNote(a),
      lebar: '15%',
      tampil: (a) => <RowBadges threshold={a} />,
    },
  ]
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <TabelData
        kolom={columns}
        baris={list.data?.ambang ?? []}
        kunciBaris={(a) => a.id}
        judul="Tangga Ambang Komite"
        keterangan={
          list.data
            ? `${list.data.total_jenjang} jenjang persetujuan aktif dari ${list.data.total} baris master.`
            : 'Memuat tangga ambang…'
        }
        labelCari="Cari lini, nama penyetuju, atau Operator ID"
        pesanKosong="Master ambang komite belum berisi satu baris pun."
  ...
  return (
    <div className={`rounded-kartu border p-4 ${style.box}`}>
      <h2 className={`flex items-center gap-2 text-sm font-semibold ${style.title}`}>
        <IkonPeringatan className={`h-4 w-4 ${style.icon}`} />
        {title}
  ...
  if (error instanceof GalatJaringan) {
    return (
      <PesanGalat
        ...
        keterangan="Tangga ambang belum dapat dimuat. Periksa koneksi lalu tekan Muat ulang."
        nada="gangguan"
      />
    )
  }
  if (error instanceof GalatAPI) {
    return <PesanGalat judul="Gagal memuat" keterangan={error.message} nada="gangguan" />
  }
  return (
    <PesanGalat
      judul="Gagal memuat"
      keterangan="Tangga ambang belum dapat dimuat. Coba tekan Muat ulang."
      nada="gangguan"
    />
  )
}

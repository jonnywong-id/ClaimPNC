import type { Rekening } from '@/api/tipe'

type Props = {
  baris: Rekening[]
  memuat: boolean
  /** Tindakan per baris; kosong berarti tabel hanya dibaca. */
  aksi?: ((rekening: Rekening) => React.ReactNode) | undefined
}
 ...
 */
export function TabelRekening({ baris, memuat, aksi }: Props) {
  if (memuat) {
          ...
          {baris.map((r) => (
            <tr key={`${r.kode_bank}-${r.nomor_rekening}`} className="border-b border-slate-200">
              ...
              <td className="py-2 pr-4">
                <LencanaStatus rekening={r} />
              </td>
              <td className="py-2 pr-4">
                <KeteranganKasir rekening={r} />
              </td>
 ...
 */
function LencanaStatus({ rekening }: { rekening: Rekening }) {
  const warna =
    rekening.status === '1'
 ...
 */
function KeteranganKasir({ rekening }: { rekening: Rekening }) {

...
func getQuery(name string) string {
	...
	if !existing {
		panic(fmt.Sprintf("masterxol/sqlstore: kueri %q tidak ditemukan di berkas .sql", name))
	}
	return text
}
...
func loadAllQueries() map[string]string {
	...
	if err != nil {
		panic("masterxol/sqlstore: tidak dapat membaca berkas kueri: " + err.Error())
	}
	for _, files := range list {
		content, err := queryFiles.ReadFile(files.Name())
		if err != nil {
			panic("masterxol/sqlstore: tidak dapat membaca " + files.Name() + ": " + err.Error())
		}
		for name, text := range splitByName(string(content)) {
			if _, duplicate := result[name]; duplicate {
				panic("masterxol/sqlstore: nama kueri ganda: " + name)
			}
			result[name] = text
		}
	}

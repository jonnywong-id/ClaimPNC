// Package sqlstore memenuhi seam mastersurveyors.Repo dengan SQL terhadap Oracle.
//
...
func getQuery(name string) string {
	...
	if !existing {
		panic(fmt.Sprintf("mastersurveyors/sqlstore: kueri %q tidak ditemukan di berkas .sql", name))
	}
	return text
}
...
func loadAllQueries() map[string]string {
	...
	if err != nil {
		panic("mastersurveyors/sqlstore: tidak dapat membaca berkas kueri: " + err.Error())
	}
	for _, file := range list {
		content, err := queryFiles.ReadFile(file.Name())
		if err != nil {
			panic("mastersurveyors/sqlstore: tidak dapat membaca " + file.Name() + ": " + err.Error())
		}
		for name, text := range splitByName(string(content)) {
			if _, clash := result[name]; clash {
				panic("mastersurveyors/sqlstore: nama kueri ganda: " + name)
			}
			result[name] = text
		}
	}

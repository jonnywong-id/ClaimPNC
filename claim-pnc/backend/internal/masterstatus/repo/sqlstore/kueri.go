...
func ambilKueri(nama string) string {
	teks, ada := kueri[nama]
	if !ada {
		panic(fmt.Sprintf("masterstatus/sqlstore: kueri %q tidak ditemukan di berkas .sql", nama))
	}
	return teks
}
...
func muatSeluruhKueri() map[string]string {
	...
	for _, berkas := range daftar {
		...
		if err != nil {
			panic("masterstatus/sqlstore: tidak dapat membaca " + berkas.Name() + ": " + err.Error())
		}
		for nama, teks := range pecahPerNama(string(isi)) {
			if _, bentrok := hasil[nama]; bentrok {
				panic("masterstatus/sqlstore: nama kueri ganda: " + nama)
			}
			hasil[nama] = teks
		}
	}

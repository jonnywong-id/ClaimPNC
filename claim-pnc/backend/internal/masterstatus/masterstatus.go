...
// MaxLabelLength membatasi panjang label status.
//
...
// ClaimStatus adalah satu baris master status.
//
...
type ClaimStatus struct {
	// Code adalah LSC_ID. Dibuat sistem saat status ditambahkan dan TIDAK PERNAH
	...
	// LegacyCode adalah OLD_LSC_ID, penomoran `01`–`11` yang melekat pada sebelas kode
...
func (s ClaimStatus) Clean() ClaimStatus {
	return ClaimStatus{
		Code:       strings.TrimSpace(s.Code),
		Label:      strings.TrimSpace(s.Label),
		LegacyCode: strings.TrimSpace(s.LegacyCode),
	}
}

// LabelKey adalah bentuk label yang dipakai menguji keunikan.
//
...
type Repo interface {
	...
	// Ambil mengembalikan satu status. Mengembalikan ErrNotFound bila kodenya
	// tidak ada.
	Get(ctx context.Context, code string) (ClaimStatus, error)
	...
	Insert(ctx context.Context, label string) (ClaimStatus, error)
...
func CheckLabel(label string) []Violation {
	...
	if bersih == "" {
		...
		pelanggaran = append(pelanggaran, Violation{
			Field:   FieldLabel,
	...
	if length := utf8.RuneCountInString(bersih); length > MaxLabelLength {
		pelanggaran = append(pelanggaran, Violation{
			Field:   FieldLabel,
			Message: "Status paling panjang " + itoa(MaxLabelLength) + " karakter.",
		})
	}

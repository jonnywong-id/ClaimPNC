...
func query(name string) string {
	...
	if !exists {
		panic(fmt.Sprintf(
			"inboxclaimtreatyprop/sqlstore: kueri %q tidak ditemukan di berkas .sql", name))
	}
	return text
}
...
func loadQueries() map[string]string {
	...
	if err != nil {
		panic("inboxclaimtreatyprop/sqlstore: tidak dapat membaca berkas kueri: " + err.Error())
	}

	for _, entry := range entries {
		content, err := queryFiles.ReadFile(entry.Name())
		if err != nil {
			panic("inboxclaimtreatyprop/sqlstore: tidak dapat membaca " +
				entry.Name() + ": " + err.Error())
		}
		for name, text := range splitByName(string(content)) {
			if _, clash := result[name]; clash {
				panic("inboxclaimtreatyprop/sqlstore: nama kueri ganda: " + name)
			}
			result[name] = text
		}
	}

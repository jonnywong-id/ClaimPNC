// Package inboxclaimtreatynonprophttp adalah lapisan transport modul Inbox Claim Treaty
// Non Prop.
//
...
// sebaliknya — dan di modul ini bocornya akan nyata: WorkItem membawa Reference dan
// AssignedOperator, dua isian yang tidak pernah digambar sebagai kolom.
...
// properti Pega: `ceding_co`, bukan `cari16`.
//
...
type WorkItemDTO struct {
	...
	// bilangan.
	AgingDays int `json:"aging"`

	CreateOperator     string `json:"operator_pembuat"`
...
type TabDTO struct {
	...
	// kebenaran — dan supaya kedua checkbox tidak digambar pada tab yang tidak
	// mengenalnya.
	ScopedToCaller  bool `json:"hanya_milik_saya"`
	SupportsSeeAll  bool `json:"pakai_lihat_semua"`
	SupportsTBAOnly bool `json:"pakai_lihat_tba"`
...
// MetadataResponse adalah jawaban GET /api/inbox-claim-treaty-non-prop/tab.
...
// sebuah checkbox mengembalikan false, dan layar dapat mematikan centangnya alih-alih
...
type AppliedFilterDTO struct {
	...
	TBAOnly bool `json:"lihat_tba"`
}

// ListResponse adalah jawaban GET /api/inbox-claim-treaty-non-prop.
...
func toWorkItemDTO(item inboxclaimtreatynonprop.WorkItem) WorkItemDTO {
	return WorkItemDTO{
		Reference:          item.Reference,
		ClaimID:            item.ClaimID,
		MasterID:           item.MasterID,
		JSONMasterID:       item.JSONMasterID,
...
func toWorkItemListDTO(items []inboxclaimtreatynonprop.WorkItem) []WorkItemDTO {
...
func toTabDTO(tab inboxclaimtreatynonprop.Tab) TabDTO {
	...
	return TabDTO{
		...
		SupportsTBAOnly: tab.SupportsTBAOnly,
		Blocked:         tab.Blocked,
...
func toPaginationDTO(page inboxclaimtreatynonprop.Page) PaginationDTO {
...
func toListResponse(listed usecase.Listed, portalAlias string) ListResponse {
	return ListResponse{
		...
		Filter: AppliedFilterDTO{
			SeeAll:  listed.Query.SeeAll,
			TBAOnly: listed.Query.TBAOnly,
		},
		Portal: portalAlias,
	}
}

...
// Galat domain modul Inbox Manager Receive / PUCL.
//
...
var (
	...
	//
	// Di layar ini ia bukan soal penyaringan — tidak satu pun kueri menyaring menurut
	// pemanggil. Ia soal jejak: seluruh baris layar ini memuat nama tertanggung dan nomor
	// polis milik pekerjaan orang lain, dan selama pemeriksaan peran belum ada
	// (`TKT-F3-004`), catatan siapa yang membukanya adalah satu-satunya kontrol yang
	// tersisa.
	ErrCallerUnknown = errors.New(
		"inboxmanagerreceivepucl: identitas pemanggil tidak terbaca")

	// ErrWriteNotAvailable berarti aksi tulis diminta pada modul yang hanya membaca.
	//
	// Ia ADA supaya aksi yang di layar lama menulis dapat dijawab dengan alasan alih-alih
...
func (e *ValidationError) Error() string {
	if len(e.Violations) == 0 {
		return "inboxmanagerreceivepucl: isian tidak sah"
	}
	...
	return "inboxmanagerreceivepucl: " + strings.Join(parts, "; ")
}

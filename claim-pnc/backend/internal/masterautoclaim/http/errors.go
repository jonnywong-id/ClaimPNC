...
const (
	...
	// CodeInitialTaken dibedakan dari CodeValidationFailed dengan sengaja.
	//
	// Ia konflik KEADAAN, bukan isian yang cacat: pengguna memilih sumber bisnis yang
	// sah dari lookup, dan yang salah hanyalah bahwa sumber bisnis itu sudah punya
	// baris. Perbaikannya bukan "betulkan isian" melainkan "buka baris yang sudah ada",
	// dan layar menanganinya berbeda.
	CodeInitialTaken = "sumber_bisnis_sudah_ada"
...
func mapError(err error) (int, ErrorResponse, bool) {
	...
	case errors.Is(err, masterautoclaim.ErrInitialTaken):
		// 409, bukan 422. Padanan pesan "Data sudah pernah diinput." pada
		// Activity/ValidasiAutoClaim step 5.
		return http.StatusConflict, ErrorResponse{
			Code:    CodeInitialTaken,
			Message: "Sumber bisnis ini sudah ada di Master Auto Claim. Buka barisnya untuk mengubah, bukan menambahkannya lagi.",
		}, true

	case errors.Is(err, masterautoclaim.ErrUnknownStatus):
	...
	case errors.Is(err, masterautoclaim.ErrNotFound):
		return http.StatusNotFound, ErrorResponse{
			Code:    CodeNotFound,
			Message: "Baris Master Auto Claim yang dimaksud tidak ditemukan. Mungkin sudah diubah petugas lain — muat ulang daftarnya.",
		}, true

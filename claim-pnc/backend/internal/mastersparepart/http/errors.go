...
const (
	...
	// CodeKeyTaken dibedakan dari CodeValidationFailed dengan sengaja.
	//
	// Ia konflik KEADAAN, bukan isian yang cacat: nomor, nama, atau kodenya sah, dan yang
	// salah hanyalah bahwa nilai itu sudah dipakai sparepart lain. Perbaikannya bukan
	// "betulkan isian" melainkan "buka baris yang sudah ada, atau pakai nilai lain", dan
	// layar menanganinya berbeda.
	//
	// SATU kode untuk ketiga kunci alami, bukan tiga kode terpisah. Yang membedakan ketiganya
	// adalah isian mana yang disorot, dan itu sudah disampaikan lewat `detail[].kolom` —
	// tiga kode hanya akan menuntut layar membedakan hal yang tidak perlu dibedakan.
	CodeKeyTaken = "kunci_sparepart_sudah_ada"
...
func mapError(err error) (int, ErrorResponse, bool) {
	var validationError *mastersparepart.ValidationError
	...
	case errors.As(err, &validationError):
		...
		// `Activity/ValidateMasterSparepart` pun menyusun tiga pesan berdampingan lalu
	...
	case errors.Is(err, mastersparepart.ErrNumberTaken):
		status, body := keyTaken("nomor_sparepart", "Nomor sparepart")
		return status, body, true

	case errors.Is(err, mastersparepart.ErrNameTaken):
		status, body := keyTaken("nama_sparepart", "Nama sparepart")
		return status, body, true

	case errors.Is(err, mastersparepart.ErrCodeTaken):
		status, body := keyTaken("kode_sparepart", "Kode sparepart")
		return status, body, true

	case errors.Is(err, mastersparepart.ErrUnknownStatus):
	...
	case errors.Is(err, mastersparepart.ErrNotFound):
		return http.StatusNotFound, ErrorResponse{
			Code:    CodeNotFound,
			Message: "Sparepart yang dimaksud tidak ditemukan. Mungkin sudah diubah petugas lain — muat ulang daftarnya.",
		}, true
...
// keyTaken menyusun jawaban 409 untuk salah satu dari ketiga kunci alami.
//
// 409, bukan 422. Padanan pesan `Activity/UpdateSparepartHE_act`: "Nama sparepart sudah
// ada". Ia konflik keadaan, bukan isian yang cacat; lihat CodeKeyTaken.
func keyTaken(field, label string) (int, ErrorResponse) {
	return http.StatusConflict, ErrorResponse{
		Code:    CodeKeyTaken,
		Message: label + " tersebut sudah dipakai. Silakan ganti dengan yang lain.",
		Detail: []ViolationDTO{{
			Field:   field,
			Message: label + " tersebut sudah dipakai sparepart lain.",
		}},
	}
}

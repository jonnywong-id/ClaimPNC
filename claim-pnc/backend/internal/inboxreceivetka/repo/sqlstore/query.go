// Package sqlstore memenuhi seam inboxreceivetka.Repo dengan SQL terhadap Oracle.
//
...
//     teks SQL — termasuk nilai status pekerjaan, yang merupakan konstanta di dalam kode
//     kita sendiri. Aturan yang mengenal pengecualian bukan aturan.
package sqlstore
...
func getQuery(name string) string {
	...
	if !existing {
		panic(fmt.Sprintf(
			"inboxreceivetka/sqlstore: kueri %q tidak ditemukan di berkas .sql", name))
	}
	return text
}
...
func loadAllQueries() map[string]string {
	...
	if err != nil {
		panic("inboxreceivetka/sqlstore: tidak dapat membaca berkas kueri: " + err.Error())
	}
	for _, entry := range list {
		content, err := queryFiles.ReadFile(entry.Name())
		if err != nil {
			panic("inboxreceivetka/sqlstore: tidak dapat membaca " + entry.Name() +
				": " + err.Error())
		}
		for name, text := range splitByName(string(content)) {
			if _, clash := result[name]; clash {
				panic("inboxreceivetka/sqlstore: nama kueri ganda: " + name)
			}
			result[name] = text
		}
	}

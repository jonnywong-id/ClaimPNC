...
-- (statusprogres.sql). Tingkat dua memakai nama yang lebih pendek. Keduanya sekerabat,
...
--
-- Empat aturan yang mengikat seluruh berkas ini, sama seperti statusprogres.sql:
...
-- tersimpan; alasan lengkapnya ada pada doc comment statusprogres.Repo2.

-- name: statusprogres2_daftar
--
...
--
-- TRIM pada penyaringnya sama alasannya dengan statusprogres_ambil pada tingkat 1:
...
-- name: statusprogres2_daftar_id_terkunci
--
...
-- yang sama diturunkan — bentuk nomornya tetap sama (statusprogres.FormatNomor2).
SELECT ID_MST
  FROM POOLDATA.GCNM_MST_PROGRESS
 FOR UPDATE

-- name: statusprogres2_sisip
--
...
-- name: statusprogres2_periksa_tabel
--
-- Memastikan tabel ada dan dapat dibaca akun aplikasi, tanpa mengambil satu baris pun.
-- Dipakai mode periksa, mengikuti pola statusprogres_periksa_tabel.
SELECT ID_MST
  FROM POOLDATA.GCNM_MST_PROGRESS
 WHERE 1 = 0

...
//
// Berkas ini melengkapi statusprogres.go, yang sejak awal sudah menyebut bahwa tingkat
...
// dapat dilakukan tanpa membaca tingkat 1 lebih dulu (lihat Isian2 dan Repo2.SisipBaru).
//
...
//	STS_PROGRESS1  AS "City"        -> NamaInduk   (bukan nama kota)
//	STS_PROGRESS2  AS "CityID"      -> Nama        (bukan kode kota)
//	ID_PROGRESS    AS "District"    -> IDInduk     (bukan kabupaten)
...
// StatusProgres2 adalah satu baris master status progres tingkat 2.
type StatusProgres2 struct {
	...
	// IDInduk merujuk Status Progres 1, kolom ID_PROGRESS.
	IDInduk string

	// NamaInduk adalah SALINAN nama induk pada saat baris ini disimpan, kolom
	// STS_PROGRESS1.
	//
	// Ia salinan, bukan hasil join — dan itu perilaku sistem lama yang dipertahankan
	// apa adanya atas keputusan Work Owner 2026-09-18. Rinciannya di Repo2.SisipBaru.
	NamaInduk string
...
// Isian2 adalah nilai yang dikirim pengguna dari layar, sebelum diperiksa.
//
...
//	NamaInduk  disalin dari baris induk yang dibaca ulang
//	Tipe       tidak pernah diisi
type Isian2 struct {
	// Nama adalah keterangan status tingkat 2 yang diketik pengguna.
	Nama string

	// IDInduk adalah ID Status Progres 1 yang dipilih pengguna dari dropdown.
	IDInduk string
}

// BatasPanjangNama2 adalah panjang maksimum kolom STS_PROGRESS2.
//
// Angkanya disamakan dengan BatasPanjangNama tingkat 1, dan itu ASUMSI yang disadari —
...
// ikut berubah.
const BatasPanjangNama2 = 100

// Galat khusus tingkat 2.
var (
	// ErrIndukTidakDitemukan: Status Progres 1 yang dirujuk tidak ada.
	//
	// Ia dibedakan dari ErrTidakDitemukan supaya layar dapat mengatakan induk MANA yang
	...
	// tidak ada (404).
	ErrIndukTidakDitemukan = errors.New("statusprogres: status progres 1 induk tidak ditemukan")
)

// Bersihkan memangkas spasi di kedua ujung setiap isian.
func (i Isian2) Bersihkan() Isian2 {
	return Isian2{
		Nama:    strings.TrimSpace(i.Nama),
		IDInduk: strings.TrimSpace(i.IDInduk),
	}
}
...
// domain tidak boleh melakukannya. Pemeriksaannya ada di Repo2.SisipBaru, yang memang
// sudah membaca baris induk untuk menyalin namanya.
func (i Isian2) Periksa() error {
	...
	case i.Nama == "":
		pelanggaran = append(pelanggaran, PelanggaranIsian{
			Kolom: "nama",
			Pesan: "Nama status progres 2 wajib diisi.",
		})
	case len(i.Nama) > BatasPanjangNama2:
		pelanggaran = append(pelanggaran, PelanggaranIsian{
			Kolom: "nama",
			Pesan: fmt.Sprintf("Nama status progres 2 paling panjang %d karakter.", BatasPanjangNama2),
		})
	}
	...
	if i.IDInduk == "" {
		pelanggaran = append(pelanggaran, PelanggaranIsian{
			Kolom: "id_induk",
			Pesan: "Status Progres 1 wajib dipilih.",
		})
	}
...
// FormatNomor2 menyusun ID_MST dari nomor urut berikutnya.
//
...
//
// Memakai FormatNomor tingkat 1 di sini karena itu akan menerbitkan ID yang TIDAK dapat
...
func FormatNomor2(nomorUrut int) string {
	return fmt.Sprintf("%d", nomorUrut)
}

// Repo2 adalah seam ke penyimpanan master status progres tingkat 2 SATU portal.
//
// # Kenapa hanya Daftar dan SisipBaru
//
...
type Repo2 interface {
	// Daftar mengembalikan seluruh baris tingkat 2, terurut seperti kueri lama.
	Daftar(ctx context.Context) ([]StatusProgres2, error)

	// Ambil mengembalikan satu baris; ErrTidakDitemukan bila tidak ada.
	Ambil(ctx context.Context, id string) (StatusProgres2, error)

	// SisipBaru membaca induknya, menurunkan ID, lalu menyisipkan barisnya.
	//
	...
	// memastikan induknya ADA (ErrIndukTidakDitemukan bila tidak), dan MENYALIN namanya
	...
	SisipBaru(ctx context.Context, isian Isian2) (StatusProgres2, error)
}

// PemilihRepo2 memilih Repo2 milik satu portal entitas.
//
// Alasannya sama persis dengan PemilihRepo: portal yang tidak dikenal atau koneksinya
// belum hidup WAJIB menghasilkan galat, tidak pernah dialihkan ke portal utama sebagai
// cadangan (R-20).
type PemilihRepo2 func(portalAlias string) (Repo2, error)

...
func getQuery(name string) string {
	...
	if !exists {
		panic(fmt.Sprintf("masterpicteknik/sqlstore: kueri %q tidak ditemukan di berkas .sql", name))
	}
	return text
}
...
func loadAllQueries() map[string]string {
	...
	for _, files := range list {
		...
		if err != nil {
			panic("masterpicteknik/sqlstore: tidak dapat membaca " + files.Name() + ": " + err.Error())
		}
		for name, text := range splitByName(string(content)) {
			if _, conflict := result[name]; conflict {
				panic("masterpicteknik/sqlstore: nama kueri ganda: " + name)
			}
			result[name] = text
		}
	}

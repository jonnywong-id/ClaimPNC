...
func sampleClaims() []inboxoutstanding.OutstandingClaim {
	...
	return []inboxoutstanding.OutstandingClaim{
		...
		//     contoh yang menguji kedua lini sekaligus.
		{
			...
			BranchName:      "CILEGON",
			GroupPanel:      "006",
			BusinessGroupID: "10013", // tidak dikecualikan — terlihat NONMBU dan BONDING
...
// sampleLines memetakan pengguna contoh ke lini bisnisnya.
//
// Kuncinya HURUF BESAR — lihat komentar field `lines` pada Repo.
//
// # Dua login tiruan sengaja dibedakan
//
// Provider identitas tiruan hanya menyediakan empat login, dan MULANYA tidak satu pun
// terdaftar di sini. Akibatnya batas data per lini — aturan yang paling berbahaya bila
// salah — tidak dapat dicoba sama sekali lewat aplikasi: setiap pengguna yang dapat masuk
// jatuh ke "tanpa batas", dan layar selalu menampilkan seluruh lini.
//
// Cacat itu baru terlihat saat aplikasinya dijalankan, bukan dari uji mana pun — uji
// memanggil repo langsung dengan pengguna karangan, sehingga tidak pernah melewati
// provider identitas.
//
// Sekarang keduanya terwakili:
//
//	pictekniks  -> PA          batas data BERLAKU; hanya klaim Group Panel 002 terlihat
//	adminpnc    -> tidak ada   jatuh ke "tanpa batas" (`K-4`), seluruh lini terlihat
//
// `adminpnc` sengaja dibiarkan kosong: itulah keadaan yang paling mungkin terjadi di
// produksi sebelum admin mengisi kolom LINEBUSINESS.
func sampleLines() map[string]string {
	return map[string]string{
		"PICTEKNIKS":   inboxoutstanding.LinePA,
		"BUDISANTOSO":  inboxoutstanding.LineNonMBU,
		"SITIRAHAYU":   inboxoutstanding.LineNonMBU,
		"DEWILESTARI":  inboxoutstanding.LinePA,
		"AGUNGWIJAYA":  inboxoutstanding.LineTravel,
		"ADMINBONDING": inboxoutstanding.LineBonding,
	}
}

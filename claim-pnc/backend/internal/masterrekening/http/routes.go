...
//
// SELURUH RUTE DI SINI TERLINDUNGI. Caller wajib memasangnya di dalam grup yang
...
func Mount(r chi.Router, h *Handler) {
	r.Route("/master-rekening", func(routes chi.Router) {
		...
		routes.Get("/{kodeBank}/{nomorRekening}", h.Get)
		routes.Put("/{kodeBank}/{nomorRekening}", h.Update)
		routes.Post("/{kodeBank}/{nomorRekening}/keputusan", h.Decide)
	})
}

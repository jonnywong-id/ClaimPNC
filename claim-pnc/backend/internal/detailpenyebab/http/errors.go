...
const (
	...
	// CodeIDTaken dibedakan dari CodeValidationFailed dengan sengaja.
	//
	// Ia BUKAN kesalahan pengguna sama sekali: ID-nya diterbitkan server dari sequence
	// `D_CAUSE_SEQ`, dan bentroknya berarti deret itu sudah pernah dipakai — misalnya
	// karena sequence di-reset, atau karena baris lama disisipkan dengan ID yang dikarang.
	//
	// Perbaikannya ada di tangan DBA, bukan di tangan petugas, dan pesannya harus
	// mengatakan demikian alih-alih menyuruh petugas membetulkan isian yang sudah benar.
	//
	// Namanya `kunci_detail_penyebab_sudah_ada` supaya ia sebentuk dengan
	// `kunci_login_surveyor_sudah_ada` dan `kunci_sparepart_sudah_ada` — ketiganya
	// menyatakan hal yang sama, yaitu kunci yang bentrok.
	CodeIDTaken = "kunci_detail_penyebab_sudah_ada"
)
...
func mapError(err error) (int, ErrorResponse, bool) {
	...
	case errors.Is(err, detailpenyebab.ErrIDTaken):
		// 409, bukan 422 — ia konflik keadaan, bukan isian yang cacat; lihat CodeIDTaken.
		return http.StatusConflict, ErrorResponse{
			Code: CodeIDTaken,
			Message: "Nomor ID yang diterbitkan sistem sudah dipakai baris lain. " +
				"Baris ini belum tersimpan.",
			Detail: []ViolationDTO{{
				Field: "id",
				Message: "Isian Anda sudah benar — yang bermasalah adalah penomorannya. " +
					"Coba simpan sekali lagi; bila tetap ditolak, hubungi administrator " +
					"Claim PNC untuk memeriksa sequence D_CAUSE_SEQ.",
			}},
		}, true

	case errors.Is(err, detailpenyebab.ErrNotFound):
		return http.StatusNotFound, ErrorResponse{
			Code: CodeNotFound,
			Message: "Detail penyebab kerugian yang dimaksud tidak ditemukan. " +

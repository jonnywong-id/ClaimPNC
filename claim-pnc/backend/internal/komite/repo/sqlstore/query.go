// Package sqlstore memenuhi seam komite.Repo dengan SQL terhadap Oracle.
//
...
//     dalam teks SQL.
//
// Ditambah satu aturan khusus modul ini: TIDAK ADA pernyataan yang menulis. Master
// ambang komite dibaca saja selama masa paralel — lihat catatan kepala pada threshold.sql.
package sqlstore
...
func query(name string) string {
	...
	if !ok {
		panic(fmt.Sprintf("komite/sqlstore: kueri %q tidak ditemukan di berkas .sql", name))
	}
	return text
}
...
func loadAllQueries() map[string]string {
	...
	if err != nil {
		panic("komite/sqlstore: tidak dapat membaca berkas kueri: " + err.Error())
	}
	for _, entry := range entries {
		content, err := queryFiles.ReadFile(entry.Name())
		if err != nil {
			panic("komite/sqlstore: tidak dapat membaca " + entry.Name() + ": " + err.Error())
		}
		for name, text := range splitByName(string(content)) {
			if _, conflict := result[name]; conflict {
				panic("komite/sqlstore: nama kueri ganda: " + name)
			}
			result[name] = text
		}
	}
